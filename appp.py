...
dataset_url = 'https://drive.google.com/uc?id=16EV2Pz8pkr973dowSI7bnEGy-8Xzc2Gx'
...
def download_files():
    if not os.path.exists('Teepublic_review.csv'):
        gdown.download(dataset_url, 'Teepublic_review.csv', quiet=False)
    if not os.path.exists('vectorizer.joblib'):
        gdown.download(vectorizer_url, 'vectorizer.joblib', quiet=False)
    if not os.path.exists('svm_model.joblib'):
        gdown.download(model_url, 'svm_model.joblib', quiet=False)

# Call download function at start
download_files()

# Load your data
@st.cache_data
def load_data():
    return pd.read_csv('Teepublic_review.csv', encoding="latin1")
